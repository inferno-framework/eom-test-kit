module EOMTestKit
    class EOMReadCancerConditionTest < Inferno::Test
        id :eom_read_cancer_condition_test
        title 'Receive EOM Primary Cancer Condition Resource'
        description %(
        ...
        input :condition_data,
            title: 'EOM Primary Cancer Condition Data',
            optional: true,
            description: %q(
        ...
        run do
            omit_if !condition_data.present?, "No data provided; skipping test."

            # if the input is a url, else it is a json
            if condition_data.strip[0] != '{'
                assert_valid_http_uri(condition_data, "Location provided is not a valid http uri.")
                get condition_data
                assert_response_status(200)
                assert_valid_json(request.response_body, "Data received from request is not a valid JSON")
                data = request.response_body
            else
                assert_valid_json(condition_data, "JSON inputted was not in a valid format")
                data = condition_data
            end
